...
;Interface Settings
  ...
  !define MUI_WELCOMEPAGE_TEXT  "This wizard will guide you through the installation of ParacraftSDK. It is recommended that you close all other applications before starting Setup.Note to Win2k/XP users: You may require administrator privileges to install Paracraft successfully."
  ;!define MUI_WELCOMEFINISHPAGE_BITMAP "Texture\ParacraftSDK\brand\installer.bmp"
  !define MUI_HEADERIMAGE
  ;!define MUI_HEADERIMAGE_BITMAP  "Texture\ParacraftSDK\brand\header.bmp"
...
;Language Selection Dialog Settings
  ...
  !define MUI_LANGDLL_REGISTRY_KEY "Software\ParaEngine\ParacraftSDK" 
...
;Pages
  ...
  !define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\ParaEngine\ParacraftSDK" 
  !define MUI_STARTMENUPAGE_REGISTRY_VALUENAME "ParacraftSDK"
...
;-----------------------------------------------------------------------------------------------------  
!define PROGRAM_NAME "ParacraftSDK"
...
LangString Name ${LANG_ENGLISH} "ParacraftSDK"
LangString Name ${LANG_SIMPCHINESE} "ParacraftSDK开发包"
...
LangString Caption ${LANG_ENGLISH} "ParacraftSDK v1"
LangString Caption ${LANG_SIMPCHINESE} "ParacraftSDK"
...
#Icon ${ProgramIcon}
#UninstallIcon "Texture\ParacraftSDK\brand\uninstaller.ico"
...
running:
	MessageBox  MB_ICONSTOP  "ParacraftSDK已在运行,请将其关闭并重新安装!"
	Quit
notrun:	
...
section
	...
	SetOutPath "$INSTDIR"
	File ..\ParacraftSDK.sln
	StrCpy $ShortCutLinkPath  "$INSTDIR\ParacraftSDK.sln"
	
	File /x *.opensdf /x *.sdf /x *.suo /x *.user ..\*.* 
	
	SetOutPath "$INSTDIR\proj.vs"
	File /r /x CMakeFiles /x *.opensdf /x *.sdf /x *.suo /x *.user /x Win32 ..\proj.vs\*.*
	
	SetOutPath "$INSTDIR\bin"
	File /r ..\bin\*.*
	
	SetOutPath "$INSTDIR\samples"
	File /r /x .git ..\samples\*.*
	
	SetOutPath "$INSTDIR\src"
	File /r /x .git /x CMakeFiles /x *.opensdf /x *.sdf /x *.suo /x *.user /x Win32 ..\src\*.*
	
	SetOutPath "$INSTDIR\redist"
	File ..\redist\ParaCraft.exe
	...
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ParacraftSDK" \
                 "DisplayName" "ParacraftSDK"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ParacraftSDK" \
