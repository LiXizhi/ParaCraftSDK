		...
		*/
		void Init(const char* sFilename)
		{
			...
			if (m_hDLL != NULL)
			{
				...
				{
#ifdef	WIN32
					printf("failed loading %s : because it does not expose the LibDescription method\r\n", sDLLPath.c_str());
#else
					const char* sErrorMsg = dlerror();
					if(sErrorMsg == 0)
						sErrorMsg = "unknown error";
					printf("warning: GetProcAddress( %s ) failed because %s\n", sDLLPath.c_str(), sErrorMsg);
#endif
					// handle the error
					UnLoad();
				}
				...
				{
#ifdef	WIN32
					printf("failed loading %s : because it does not expose the LibVersion method\r\n", sDLLPath.c_str());
#else
					const char* sErrorMsg = dlerror();
					if(sErrorMsg == 0)
						sErrorMsg = "unknown error";
				...
				if (pLibNumberClasses != 0)
				{
					...
					//printf("lib classes count %d \r\n", nClassNum);
					m_listClassDesc.reserve(nClassNum);
				}
				else
				{
#ifdef	WIN32
					printf("failed loading %s : because it does not expose the LibNumberClasses method\r\n", sDLLPath.c_str());
#else
					const char* sErrorMsg = dlerror();
					if(sErrorMsg == 0)
						sErrorMsg = "unknown error";
				...
				if (pLibDescription != 0)
				{
					...
					for (int i=0; i<nClassNum; ++i)
					{
						...
						{
							printf("the %d th class in %s is not loaded \r\n", i, sDLLPath.c_str());
						}
					}
				}
				else
				{
#ifdef	WIN32
					printf("failed loading %s : because it does not expose the LibClassDesc method\r\n", sDLLPath.c_str());
#else
					const char* sErrorMsg = dlerror();
					if(sErrorMsg == 0)
						sErrorMsg = "unknown error";
			...
			if(IsValid())
			{
				printf("Plug-in loaded: %s\r\n", sDLLPath.c_str());
			}
			else
			{
				printf("Failed loading plug-in: %s\r\n", sDLLPath.c_str());
			}
		}
