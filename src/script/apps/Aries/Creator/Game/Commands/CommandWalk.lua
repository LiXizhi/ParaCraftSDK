...
Commands["walk"] = {
	name="walk", 
	quick_ref="/walk [@entityname] [x y z] [-random 5] [-speed 1.0] [-finishanim 0] [-to|away @entityname] [-dist 10]", 
...
@param -speed: if specified it will modify the entity's speedscale. 
...
/walk -away @a -dist 10 walk away from nearby player until distance is 10. 
	...
	handler = function(cmd_name, cmd_text, cmd_params, fromEntity)
		...
		local random_radius, speedscale, finish_anim, to_x, to_y, to_z, isAway, dist;
		...
		while (option) do
			...
			elseif(option == "speed") then	
				speedscale, cmd_text = CmdParser.ParseNumber(cmd_text);
