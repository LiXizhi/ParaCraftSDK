...
local TimeSpan = {
	...
	framerate = CommonCtrl.Animation.Motion.AnimationEditor.AnimationEditor_Config.framerate,
}
commonlib.setfield("CommonCtrl.Animation.Motion.TimeSpan",TimeSpan);
...
commonlib.setfield("CommonCtrl.Animation.Motion.Util",Util);
...
function Util.DeleteMeshPhysicsObject(animationKeyFrames,result,curKeyframe,frame)
	...
	local KeyFramesPool = CommonCtrl.Animation.Motion.KeyFramesPool;
		local TargetName = animationKeyFrames.TargetName;	
...
function Util.GetDisplayObj(name)
	...
	if(display:IsValid()==false)then
		--log("warning: animator binding control:"..name.." is not found\n");
		display = nil;
	end
	return display;
end
...
------------------------------------------------------------
local KeyFramesPool = {};
commonlib.setfield("CommonCtrl.Animation.Motion.KeyFramesPool",KeyFramesPool);
	...
	--local sceneName = self.TargetName;
			...
			for obj_params,vv in pairs(keyframes) do
				...
				if(property == "DiscreteObjectKeyFrame")then
					...
					if(keytime)then
						local k_frame = CommonCtrl.Animation.Motion.TimeSpan.GetFrames(keytime);
