...
-- @param bForceRun: if true we will always run profiling regardless if profiling is enabled or not globally. 
function npl_profiler.perf_begin(name, bForceRun, bRecursive)
	if(not perf_enabled and not bForceRun) then
		return;
	end
...
-- nested calls with the same name are supported, where only the outer is calculated. 
-- @param bForceRun: if true we will always run profiling regardless if profiling is enabled or not globally. 
function npl_profiler.perf_end(name, bForceRun)
	if(not perf_enabled and not bForceRun) then
		return;
	end
	local stat = perf_stats[name]
