...
function AssetManifestPage.OnInit()
	...
	if not fin then
      commonlib.echo("cannot open _assetmanifest.ftp.uploader0.txt!");	
	end;
...
function AssetManifestPage.OnClickGenerate(name, values)
	local s=values.skipRegEx;
	local p_id=0;
	local patternAssetRequested="AssetRequested ";
	local patternOpenAssetFile="ParaFile%.OpenAssetFile using local file:";
	local patternTerrainOnLoad="Terrain%.OnLoad% using local file:";
	local patternDBEntity="DBEntity%.PrepareDatabaseFile using local file:";
	
	if (values.checkAssetLoaded or values.checkOpenAssetFile  or values.checkTerrainOnLoad  or values.checkDBEntity) then
	
		ds0={};
		getSkipList(s,ds0);

		local cp_id=ParaIO.CopyFile("assets.log","assets_ftp.log",true);
		if not cp_id then
			commonlib.echo("copy assets.log failed!"); 
		end;

		local ff0=ParaIO.DoesFileExist("assets_ftp_success.txt");
		if not ff0 then
			fw=ParaIO.open("_assetmanifest.ftp.list.txt","w");
			fw:WriteString("");
			fw:close();
			commonlib.echo("clear _assetmanifest.ftp.list.txt!"); 
		else
			commonlib.echo("assets_ftp_success.txt exist! Fail clear _assetmanifest.ftp.list.txt!"); 
		end;

		cp_id=ParaIO.CopyFile("_assetmanifest.ftp.uploader.txt","_assetmanifest.ftp.uploader0.txt",true);
		if not cp_id then
			commonlib.echo("copy _assetmanifest.ftp.uploader.txt failed!"); 
		end;		

		local fin=ParaIO.open("assets_ftp.log","r");
		local fw=ParaIO.open("_assetmanifest.ftp.uploader0.txt","a");

		if not fin then
			commonlib.echo("open assets_ftp.log failed!"); 
		end;
		
		fw:WriteString("");   

		-- use to identify if the directory is created or not, dd[dir]=nil not created, dd[dir]=1 created
		dd={} 
		fr=ParaIO.open("_assetmanifest.ftp.list.txt","r");
		local s_fr=fr:GetText();
		fr:close();
		if (not s_fr) then
			s_fr="";
		end;

		fr=ParaIO.open("_assetmanifest.ftp.uploader.txt","r");
		local s_fr1=fr:GetText();
		fr:close();
		if (not s_fr1) then
			s_fr1="";
		end;
...
--		    commonlib.echo(lines);
			...
			p_id=0;    
			f_l=0;
			if (values.checkAssetLoaded and string.find(lines,patternAssetRequested) ) then 
				_, f_l=string.find(lines,patternAssetRequested);
				p_id=1;
			elseif (values.checkOpenAssetFile and string.find(lines,patternOpenAssetFile)) then 
				_, f_l=string.find(lines,patternOpenAssetFile);
				p_id=1;
			elseif (values.checkTerrainOnLoad and string.find(lines,patternTerrainOnLoad)) then 
				_, f_l=string.find(lines,patternTerrainOnLoad);
				p_id=1;
			elseif (values.checkDBEntity and string.find(lines,patternDBEntity)) then 
				_, f_l=string.find(lines,patternDBEntity);
				p_id=1;
			end;
			
			if (p_id==1) then
				s_f=string.sub(lines,f_l+1);
				d0={};
				getdir(d0,s_f);
				dn=table.getn(d0)
				for i=0,dn-1 do
					...
					if not dd[dr] then
						dd[dr]=1;
						w_id=judgeWrite(dr,ds0,s_fr);  
						w_id0=IsInFArray(dr,dg);  
						if (w_id) and (not w_id0) then
				--			commonlib.echo(tostring(w_id0).."|"..dr);						
							fw:WriteString(dr.."\n");   
						end            
					end  -- if not dd[dr]
				end -- for i=      
			...
			end --if string.find(lines,pattern1)         
		end -- while   
	--    commonlib.echo({dd});	
		fw:close();
		fin:close();
	...
	-- update textarea of tab "publish"
		...
		page:SetValue("flist", flists);  
		_guihelper.MessageBox("_assetmanifest.ftp.uploader0.txt is generated successfully, please goto publish tab to modify or upload these files!");
	
	else
--		_guihelper.MessageBox({values});
		_guihelper.MessageBox("You should select 1 choice item at least!");
	end;
end

-- Save modified textarea contents into file "_assetmanifest.ftp.uploader0.txt"
function AssetManifestPage.OnClickSaveFile(name, values)
	local s=values.flist;   
	local p1,lines,x_s = 0,"","";
	local fw=ParaIO.open("_assetmanifest.ftp.uploader0.txt","w");
	local d_g={};
	local pattern1="^(.*)/(.*)$";
	local pattern2="^%[search%] (.*)/(.*)$";
	local pattern21="^%[search1%] (.*)/(.*)$";
...
-- update textarea of tab "publish"
	...
	local page = document:GetPageCtrl();
	page:SetValue("flist", flists);  
	_guihelper.MessageBox("_assetmanifest.ftp.uploader0.txt is saved successfully");
end
...
--[[		
    ...
    fw:WriteString("\n");
    fw:close();
	local fw=ParaIO.open("ftpget.txt","w");
    fw:WriteString("\n");
    fw:close();
]]    
...
function AssetManifestPage.OnClickGenBackgroundLoaderFile(name, values)
	local path = "assets.log";
	
	local cp_id=ParaIO.CopyFile(path,"assets.tmp.log",true);
	if not cp_id then
		commonlib.echo("copy assets.log failed!"); 
	end;
	path = "assets.tmp.log";
	...
	local output_file = "temp/assets.background_loader.log"; -- page:GetValue("bgloader_output_file", "config/Aries/Preloaders/Aries_background_loader_61haqitown.txt");
	local out = ParaIO.open(output_file, "w");
	if(not file:IsValid()) then
		_guihelper.MessageBox(format("can not open file %s", path));
	elseif(not out:IsValid()) then
		_guihelper.MessageBox(format("can not open file %s", output_file));
	else
		local ext_filters = {["x"]=true, ["xml"]=true, ["dds"]=true, ["png"]=true, ["mask"]=true, ["raw"]=true, }
		-- read a line 
		local files = {};
		local line = file:readline();
		while(line) do
            local asset = string.match(line, "AssetRequested (.+)$") or string.match(line, "ParaFile%.OpenAssetFile using local file:(.+)$");
            if(asset and not asset:match("^temp/") and not asset:match("^config/")) then
				asset = string.lower(asset);
				local ext = asset:match("([^.]+)$");
				if(ext and ext_filters[ext]) then
					if( not files[asset] ) then
						files[asset] = true;
						local nFileSize = ParaIO.GetFileSize(asset)
						if(nFileSize > 0) then
							out:WriteString(format("%s,%d\r\n", asset, nFileSize));
						end
					end
				end
            end
		...
		out:close();
		file:close();
		_guihelper.MessageBox(format("output background loader file to %s", output_file), function()
		end)
	end
end
