...
local Item_HomeIndoorFurniture = {};
commonlib.setfield("Map3DSystem.Item.Item_HomeIndoorFurniture", Item_HomeIndoorFurniture)

---------------------------------
-- functions
---------------------------------

function Item_HomeIndoorFurniture:new(o)
...
function Item_HomeIndoorFurniture:OnClick(mouse_button)
	local ItemManager = Map3DSystem.Item.ItemManager;
	if(mouse_button == "left") then
		if(ItemManager.IsHomeLandEditing()) then
			local gsItem = ItemManager.GetGlobalStoreItemInMemory(self.gsid)
			if(gsItem) then
				Map3DSystem.App.HomeLand.HomeLandGateway.BuildNodeFromItem("Furniture",gsItem,self.guid);
			end
		else
...
function Item_HomeIndoorFurniture:OnModify(itemdata)
	local ItemManager = Map3DSystem.Item.ItemManager;
	if(ItemManager.IsHomeLandEditing()) then
...
function Item_HomeIndoorFurniture:OnRemove()
	local ItemManager = Map3DSystem.Item.ItemManager;
	if(ItemManager.IsHomeLandEditing()) then
...
function Item_HomeIndoorFurniture:Prepare(mouse_button)
end