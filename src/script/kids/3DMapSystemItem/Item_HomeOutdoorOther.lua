...
local Item_HomeOutdoorOther = {};
commonlib.setfield("Map3DSystem.Item.Item_HomeOutdoorOther", Item_HomeOutdoorOther)

---------------------------------
-- functions
---------------------------------

function Item_HomeOutdoorOther:new(o)
	o = o or {}   -- create object if user does not provide one
	setmetatable(o, self)
	self.__index = self
	return o
end
function Item_HomeOutdoorOther:OnClick(mouse_button)
	local ItemManager = Map3DSystem.Item.ItemManager;
	if(mouse_button == "left") then
		if(ItemManager.IsHomeLandEditing()) then
			local gsItem = ItemManager.GetGlobalStoreItemInMemory(self.gsid)
			if(gsItem) then
				Map3DSystem.App.HomeLand.HomeLandGateway.BuildNodeFromItem("OutdoorOther",gsItem,self.guid);
			end
		else
...
function Item_HomeOutdoorOther:OnModify(itemdata)
	local ItemManager = Map3DSystem.Item.ItemManager;
	if(ItemManager.IsHomeLandEditing()) then
...
function Item_HomeOutdoorOther:OnRemove()
	local ItemManager = Map3DSystem.Item.ItemManager;
	if(ItemManager.IsHomeLandEditing()) then
...
function Item_HomeOutdoorOther:Prepare(mouse_button)
end